...
function getKriteriaID($no_urut) {
	include('config.php');
	$query  = "SELECT id_ahp_kriteria FROM kriteria_edas ORDER BY id_ahp_kriteria";
	$result = mysqli_query($koneksi, $query);

	while ($row = mysqli_fetch_array($result)) {
		$listID[] = $row['id_ahp_kriteria'];
	}
...
function getAlternatifID($no_urut) {
	include('config.php');
	$query  = "SELECT id_ahp_alternatif FROM alternatif ORDER BY id_ahp_alternatif";
	$result = mysqli_query($koneksi, $query);

	while ($row = mysqli_fetch_array($result)) {
		$listID[] = $row['id_ahp_alternatif'];
	}
...
function getKriteriaNama($no_urut) {
	include('config.php');
	$query  = "SELECT nama FROM kriteria_edas ORDER BY id_ahp_kriteria";
	$result = mysqli_query($koneksi, $query);
...
function getAlternatifNama($no_urut) {
	include('config.php');
	$query  = "SELECT nama FROM alternatif ORDER BY id_alternatif";
	$result = mysqli_query($koneksi, $query);
...
function getKriteriaPV($id_kriteria) {
	include('config.php');
	$query = "SELECT bobot FROM kriteria_edas WHERE id_ahp_kriteria=$id_kriteria";
	$result = mysqli_query($koneksi, $query);
	while ($row = mysqli_fetch_array($result)) {
		$pv = $row['bobot'];
	}
...
// mencari P.Vektor kriteria
function getPVektor($id_kriteria) {
	include('config.php');
	$query = "SELECT pvektor FROM kriteria_edas WHERE id_ahp_kriteria=$id_kriteria";
	$result = mysqli_query($koneksi, $query);
	while ($row = mysqli_fetch_array($result)) {
		$pv = $row['pvektor'];
	}
...
// mencari P.Vektor kriteria
function getEI($id_kriteria) {
	include('config.php');
	$query = "SELECT ei FROM kriteria_edas WHERE id_ahp_kriteria=$id_kriteria";
	$result = mysqli_query($koneksi, $query);
	while ($row = mysqli_fetch_array($result)) {
		$pv = $row['ei'];
	}
...
// mencari jumlah matriks
function getJumlahMatriks() {
	include('config.php');
	$query  = "SELECT count(*) FROM matriks";
	$result = mysqli_query($koneksi, $query);
	while ($row = mysqli_fetch_array($result)) {
		$jmlData = $row[0];
	}
...
function getJumlahKriteria() {
	include('config.php');
	$query  = "SELECT count(*) FROM kriteria_edas";
	$result = mysqli_query($koneksi, $query);
	while ($row = mysqli_fetch_array($result)) {
		$jmlData = $row[0];
	}
...
function inputKriteriaPV ($id_kriteria,$pv) {
	...
	if (mysqli_num_rows($result)==0) {
		$query = "INSERT INTO kriteria_edas (id_ahp_kriteria, nilai) VALUES ($id_kriteria, $pv)";
	} else {
		$query = "UPDATE kriteria_edas SET bobot =$pv WHERE id_ahp_kriteria=$id_kriteria";
	}

	$result = mysqli_query($koneksi, $query);
	if(!$result) {
		echo "Gagal memasukkan / update nilai bobot kriteria";
		exit();
	}
}

// memasukkan nilai P.Vektor ke kriteria_edas
function inputNilaiPVektor($id_kriteria,$jmlmnk) {
	include ('config.php');
	...
	// jika telah ada maka diupdate
	if (mysqli_num_rows($result)==0) {
		$query = "INSERT INTO kriteria_edas (id_ahp_kriteria, pvektor) VALUES ($id_kriteria, $jmlmnk)";
	} else {
		$query = "UPDATE kriteria_edas SET pvektor =$jmlmnk WHERE id_ahp_kriteria=$id_kriteria";
	}

	$result = mysqli_query($koneksi, $query);
	if(!$result) {
		echo "Gagal memasukkan / update nilai P. Vektor kriteria";
		exit();
	}
}

// memasukkan nilai P.Vektor ke kriteria_edas
function inputNilaiEI($id_kriteria,$ei) {
	include ('config.php');
	...
	// jika telah ada maka diupdate
	if (mysqli_num_rows($result)==0) {
		$query = "INSERT INTO kriteria_edas (id_ahp_kriteria, ei) VALUES ($id_kriteria, $ei)";
	} else {
		$query = "UPDATE kriteria_edas SET ei =$ei WHERE id_ahp_kriteria=$id_kriteria";
	}

	$result = mysqli_query($koneksi, $query);
	if(!$result) {
		echo "Gagal memasukkan / update nilai Eigen Value kriteria" . mysqli_error($koneksi);;
		exit();
	}
}
...
function showTabelPerbandingan($jenis,$kriteria) {
	...
	$query = "SELECT nama FROM kriteria_edas ORDER BY id_kriteria";
	$result	= mysqli_query($koneksi, $query);
	if (!$result) {
	...
	?>
			<tr>
				<td>
					<div class="field">
						<div class="ui radio checkbox">
							<input name="pilih<?php echo $urut?>" value="1" checked="" class="show" type="radio">
				...
				<td>
					<div class="field">
						<div class="ui radio checkbox">
							<input name="pilih<?php echo $urut?>" value="2" class="show" type="radio">
